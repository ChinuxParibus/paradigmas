 ...
 */
export const PALABRAS = /[A-Z]{2,}(?=[A-Z][a-z]+[0-9]*|\b)|[A-Z]?[a-z]+[0-9]*|[A-Z]|[0-9]+/g

/**
 * Detect a valid Venezuelan phone number (area code included)
 * @type {RegExp}
 */
export const TELEFONO = /0(2(12|3[4589]|4[0-9]|[5-8][1-9]|9[1-5])|(4(12|14|16|24|26)))-?\d{7}/g

/**
 * Detect a valid email (RFC 5322 Standard)
 * @type {RegExp}
 */
export const CORREO = /[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])+/g

/**
 * Months of the year (in Spanish)
 * @return {Array}   [enero, febrero, ...]
 */
export const MESES = Array.from({ length: 12 }, (e, i) => {
 ...
 */
export const DIAS = Array.from({ length: 7 }, (e, i) => {
 ...
 * @param  {Boolean} $v1 [Any Boolean]
 * @param  {Boolean} $v2 [Any Boolean]
 * @return {Boolean}
 */
 ...
 * - If the length of props is longer than values, remaining keys will be
 *   undefined.
 * - If the length of values is longer than props, remaining values will be
 *   ignored.
 ...
 * @return {Object}             [Key-value pairs picked from object]
 */
...
/**
 * Chain multiple functions, when the result of the first function on the
 * array is the parameter of the next callback execution
 ...
 * @param  {Date}   $date [Valid Javsacript Date]
